...
if ~exist('optC','var')%var,variable,is the kind of name 'optC'
    optC='glc';
end
phiEopt='subSys';
...
if ~isfield(model,'protGroup')
    ...
    protGroup(2).phi0=0;
%     protGroup(2).phi0=0.45;%It's a typo. According to 'example_setUpModel.m' should be 0
    ...
    protGroup(4).phi0=0.45;
%     protGroup(4).phi0=0;%It's a typo. According to 'example_setUpModel.m' should be 0.45
...
% EX_glc=find(strcmp(model.rxns,'EX_glc(e)')); % glc transport
EX_glc=find(strcmp(model.rxns,'EX_glc_e_'));
...
elseif strcmp(optC,'man6p')
    EX_man6p=find(strcmp(model.rxns,'EX_man6p_e_')); % mannose-6-p transport
    rxns_C=EX_man6p;
...
elseif strcmp(optC,'mal')
    EX_mal=find(strcmp(model.rxns,'EX_mal_L_e_') | strcmp(model.rxns,'EX_mal-L(e)')); % malate
    rxns_C=EX_mal;
    model.lb(EX_mal)=-1000;
    model.lb(EX_glc)=0; 
    
 %added by Huili 20210901    
 elseif strcmp(optC,'pyr+glcn')
    EX_pyr=find(strcmp(model.rxns,'EX_pyr_e_')); % pyruvate
    EX_glcn=find(strcmp(model.rxns,'EX_glcn_e_')); % gluconate transport
    rxns_C=[EX_pyr,EX_glcn];
    model.lb(EX_glcn)=-1000;
    model.lb(EX_pyr)=-1000;
    model.lb(EX_glc)=0;
   %added by Huili 20211201   
 elseif strcmp(optC,'pyr+glyc')
    EX_pyr=find(strcmp(model.rxns,'EX_pyr_e_')); % pyruvate
    EX_glyc=find(strcmp(model.rxns,'EX_glyc_e_')); % glycerol transport
    rxns_C=[EX_pyr,EX_glyc];
    model.lb(EX_glyc)=-1000;
    model.lb(EX_pyr)=-1000;
    model.lb(EX_glc)=0;
    
%added by Huili 20210726
elseif strcmp(optC,'glc+succ')
    EX_glc=find(strcmp(model.rxns,'EX_glc_e_')); % glucose transport
    EX_succ=find(strcmp(model.rxns,'EX_succ_e_')); % succinate
    rxns_C=[EX_glc,EX_succ];
    model.lb(EX_glc)=-1000;
    model.lb(EX_succ)=-1000;
    
%added by Huili 20210823    
 elseif strcmp(optC,'succ+glcn')
    EX_succ=find(strcmp(model.rxns,'EX_succ_e_')); % succinate
    EX_glcn=find(strcmp(model.rxns,'EX_glcn_e_')); % gluconate transport
    rxns_C=[EX_succ,EX_glcn];
    model.lb(EX_glcn)=-1000;
    model.lb(EX_succ)=-1000;
    model.lb(EX_glc)=0;
    
 %added by Huili 20210831    
elseif strcmp(optC,'succ+arab')
    EX_succ=find(strcmp(model.rxns,'EX_succ_e_')); % succinate
    EX_arab=find(strcmp(model.rxns,'EX_arab_L_e_')); % arabinose transport
    rxns_C=[EX_succ,EX_arab];
    model.lb(EX_arab)=-1000;
    model.lb(EX_succ)=-1000;
    model.lb(EX_glc)=0;
    
%added by Huili 20211201  
elseif strcmp(optC,'succ+glyc')
    EX_succ=find(strcmp(model.rxns,'EX_succ_e_')); % succinate
    EX_glyc=find(strcmp(model.rxns,'EX_glyc_e_')); % glycerol transport
    rxns_C=[EX_succ,EX_glyc];
    model.lb(EX_glyc)=-1000;
    model.lb(EX_succ)=-1000;
    model.lb(EX_glc)=0;
    
%added by Huili 20211022 
    elseif strcmp(optC,'glc+lcts')
    EX_glc=find(strcmp(model.rxns,'EX_glc_e_')); % glucose transport
    EX_lcts=find(strcmp(model.rxns,'EX_lcts_e_') ); % lactose transport
    rxns_C=[EX_glc,EX_lcts];
    model.lb(EX_glc)=-1000;
    model.lb(EX_lcts)=-1000;
     
% other C-lim
elseif strcmp(optC,'lac')
    EX_lac=find(strcmp(model.rxns,'EX_lac_D_e_') | strcmp(model.rxns,'EX_lac-D(e)')); % lactate transport
    rxns_C=EX_lac;
...
elseif strcmpi(phiEopt, 'subSys')
    ...
    subSysList={'Transport Extracellular'; ...
                ...
                'Alternate Carbon Metabolism';...
                ''};
    %%2020-04-14 added by Huili            
    for i=1:length(model.subSystems)
        subarray{i}=model.subSystems{i}{1};
    end
    %%
    phiEVec=find(~ismember(subarray,subSysList));
