...
if ~isfield(model,'protGroup') % extra check on the model.Check if 'protGroup' field is in the model structure
    error('protGroup field missing. Exiting from setWeights().');
end;
if ~isfield(model,'w') % extra check on the model.Check if 'w' field is in the model structure
    model.w=zeros(length(model.rxns),1);%If w not in the model, add w field to model 
end
